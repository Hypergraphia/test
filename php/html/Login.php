        ...
        try {
            ...
            if ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                if (password_verify($password, $row['password'])) {
                    ...
                    // 認証失敗
                    $errorMessage = 'ユーザーネームあるいはパスワードに誤りがあります。';
                }
            } else {
...
if (isset($_POST["signUp"])) {
    ...
    if (!empty($_POST["username"]) && !empty($_POST["password"]) && !empty($_POST["password2"]) && $_POST["password"] === $_POST["password2"]) {
        ...
        try {
            ...
            $signUpMessage = '登録が完了しました。あなたの登録IDは '. $userid. ' です。';  // 新規登録した時の番号
        } catch (PDOException $e) {
            $errorMessage = 'データベースエラー';
            // $e->getMessage() でエラー内容を参照可能（デバッグ時のみ表示）
            // echo $e->getMessage();
...
<body>
            ...
            <div class="modalContents">
                ...
                <form id="loginForm" name="loginForm" action="" method="POST">
                    <fieldset>
                        ...
                        <input type="text" id="userid" name="userid" placeholder="ユーザーネームを入力" value="<?php if (!empty($_POST[" userid"])) {echo htmlspecialchars($_POST["userid"], ENT_QUOTES);} ?>">
                        <br>
                ...
                <form id="loginForm" name="loginForm" action="" method="POST">
                    <fieldset>
                        ...
                        <input type="text" id="username" name="username" placeholder="ユーザーネームを入力" value="<?php if (!empty($_POST[" username"])) {echo htmlspecialchars($_POST["username"], ENT_QUOTES);} ?>">
                        <br>
                        <label for="password">Password</label>
                        <input type="password" id="password" name="password" value="" placeholder="パスワードを入力">
                        <br>
                        <label for="password2">Re-type</label>
                        <input type="password" id="password2" name="password2" value="" placeholder="同じパスワードを入力">
                        <br>
    ...
    <script>
        ...
        function init() {
            ...
            var r = $i("starfield");
            r.style.position = "absolute", r.width = w, r.height = h, context = r.getContext("2d"), context.fillStyle = "rgb(255,255,255)", context.strokeStyle = "rgb(0,0,0)"
        }
